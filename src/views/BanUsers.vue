        ...
        <el-table ref="multipleTable"
            ...
            <el-table-column align="right">
                <template slot="header" slot-scope="scope">
                    <el-input v-model="search" size="mini" placeholder="请输入需要查询的客户ID" />
                </template>
...
<script>
import { banuser } from '@/api/getdata';
...
export default {
    ...
    methods: {
        ...
        currentevent(val) {
            banuser({ total: this.total, pagesize: this.pagesize, page: val }).then(res => {
    ...
    created() {
        banuser({ total: this.total, pagesize: this.pagesize, page: 1 }).then(res => {
            this.userdata = res.data.totalpagedata;
            this.total = res.data.total;
        })
    }
