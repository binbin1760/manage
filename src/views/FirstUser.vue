...
<script>
import { gooduser } from '../api/getdata';
...
export default {
    ...
    methods: {
        ...
        currentevent(val) {
            gooduser({ total: this.total, pagesize: this.pagesize, page: val }).then(res => {
    ...
    created() {
        gooduser({ total: this.total, pagesize: this.pagesize, page: 1 }).then(res => {
            this.userdata = res.data.totalpagedata;
            this.total = res.data.total;
        })
    }
